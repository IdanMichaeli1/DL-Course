...
def char_maps(text: str):
    ...
    idx_to_char = dict((i, char) for char, i in char_to_idx.items())
...
def chars_to_onehot(text: str, char_to_idx: dict) -> Tensor:
    ...
    char_indices = torch.tensor([char_to_idx[char] for char in text])
    result[torch.arange(len(text)), char_indices] = 1
...
def chars_to_labelled_samples(text: str, char_to_idx: dict, seq_len: int, device="cpu"):
    ...
    labels = torch.tensor([char_to_idx[char] for char in labels_text], device=device).view(num_samples, seq_len)
...
def hot_softmax(y, dim=0, temperature=1.0):
    ...
    result = torch.softmax(y_scaled, dim)
...
def generate_from_model(model, start_sequence, n_chars, char_maps, T):
    ...
    with torch.no_grad():
        ...
        for _ in range(n_chars - len(start_sequence)):
            encoded_out_text = chars_to_onehot(out_text[-1:], char_to_idx).unsqueeze(0).to(dtype=torch.float, device=device)
            layer_output, hidden_state = model(encoded_out_text, hidden_state)
            layer_output = hot_softmax(layer_output.squeeze(0)[-1], dim=0, temperature=T)
            predicted_char_idx = torch.multinomial(layer_output, 1).item()
            predicted_char = idx_to_char[predicted_char_idx]
            out_text += predicted_char
...
class SequenceBatchSampler(torch.utils.data.Sampler):
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        indices = list(range(n_batches * self.batch_size))
        
        for batch in range(n_batches):
            for i in range(self.batch_size):
                idx.append(indices[n_batches * i + batch])
