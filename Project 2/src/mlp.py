...
class MLP(nn.Module):
    ...
    def __init__(
        ...
        for i, (in_feat, out_feat) in enumerate(zip([in_dim, *dims[:-1]], dims)):
            
            non_linearity = nonlins[i] \
                if isinstance(nonlins[i], nn.Module) \
                else ACTIVATIONS[nonlins[i]](**ACTIVATION_DEFAULT_KWARGS[nonlins[i]])
            
            layers += [nn.Linear(in_feat, out_feat), non_linearity]
