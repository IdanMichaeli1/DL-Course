...
class MomentumSGD(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, momentum=0.9):
        ...
        self.velocities = {p: torch.zeros_like(p) for p, _ in params}
...
class RMSProp(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, decay=0.99, eps=1e-8):
        ...
        self.mavg_gradients = {p: torch.zeros_like(p) for p, _ in params}
    ...
    def step(self):
        for p, dp in self.params:
            ...
            dp += self.reg * p
            self.mavg_gradients[p] = self.decay * self.mavg_gradients[p] + (1 - self.decay) * (dp ** 2)
            p -= self.learn_rate / torch.sqrt(self.mavg_gradients[p] + self.eps) * dp